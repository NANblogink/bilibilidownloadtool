...
- **多内容类型兼容**：支持番剧、合集、课堂等类型视频的离线缓存
- **灵活的清晰度选择**：提供多种画质选项，满足不同场景需求
- **批量下载能力**：支持多集视频同时下载，提高效率
- **账号权限同步**：支持Cookie登录，仅可获取与当前账号权限一致的视频资源（需账号本身具备访问权限）
...
- **热更新功能**：支持在线检测版本更新，自动下载、校验、替换文件并重启（实验性）
- **公告系统**：支持从云端获取公告信息，在主界面顶部展示，支持关闭和版本定向推送
- **封面下载与解析**：支持在收藏夹和作品列表中一键下载视频封面，支持批量下载封面
- **收藏夹增强**：收藏夹内容卡片支持解析视频和下载封面，搜索功能优化
...
| `cloud_service.py` | 云端服务模块，负责更新检测、公告获取和热更新下载 |
...
### Cookie设置
- 如需缓存需登录才能观看的内容（如已购课程、大会员番剧等），请在"Cookie设置"区域输入B站Cookie，工具将依据该账号的权限获取资源。
...
本项目仅供学习编程技术与网络协议交流使用，**严禁用于任何商业用途**。使用本工具前，请务必阅读并遵守以下原则：

1. **权限对等原则**：本工具不具备任何破解或越权功能。下载任何受保护的视频内容，均需用户自行提供具备合法访问权限的B站账号凭证。本工具仅代替用户本地执行访问，绝不提供超出该账号原有权限的访问能力。
2. **合理使用原则**：本工具旨在为已购用户提供**个人离线观看/收听便利**，解决官方客户端暂不支持的多格式封装与离线缓存问题。下载的所有内容须保留平台原始水印及版权标识。
3. **严禁二次传播**：使用本工具下载的视频、音频、漫画等资源，**严禁以任何形式（包括但不限于上传网盘、盗版网站、二次售卖）进行公开传播**。因使用者个人行为导致的版权纠纷及法律责任，均由使用者自行承担，与开发者无关。
4. **协议约束**：请务必遵守B站用户服务协议。如平台认为本工具的离线缓存功能存在不妥，开发者将配合下架处理。

**下载即代表您已阅读、理解并同意遵守以上声明。**
...
## 版本历史
### V2.0.2
- **优化程序性能**：修复15处线程阻塞主线程导致的UI卡死问题，所有网络请求均在后台线程执行
- **增强任务记录**：所有下载类型（视频/封面/弹幕）均记录任务，新增BV号、UP主、文件大小、集数进度、开始/完成时间等字段
- **优化任务详情**：双击任务可查看完整信息，封面任务独立展示，支持多格式视频文件检测
- **移除去水印UI**：移除TV端无水印模式选项，保留平台原始水印
- **完善免责声明**：重写免责声明，强化权限对等、合理使用、严禁传播等法律条款

### V2.0.1
- **新增封面下载功能**：支持在收藏夹内容卡片和UP主作品列表中一键下载视频封面
- **新增批量封面下载**：支持批量选择收藏夹内容并一键下载所有封面
- **新增视频解析按钮**：收藏夹内容卡片新增"解析"按钮，可快速跳转解析视频
- **优化收藏夹界面**：收藏夹列表项美化，卡片式设计，搜索功能修复
- **优化DPI缩放**：调整全局DPI缩放策略，修复高DPI下按钮文字显示不全的问题
- **优化卡片文本显示**：增大文本标签宽度，移除冗余边距，解决文字左右显示不全的问题

### V2.0.0
- **新增热更新功能（实验性）**：支持在线检测更新、下载更新包、自动替换文件并重启，实现一键热更新
- **新增公告系统**：支持从云端获取公告信息，在主界面顶部展示，支持关闭和版本定向推送
- **新增云端服务模块**：新增 `cloud_service.py`，支持自建API优先、GitHub回退的双轨更新策略
- **修复已知Bug**：修复Cookie登录、程序卡顿、头像加载、退出挂起等多项已知问题
- **修复bento4缺失问题**：安装程序自动释放bento4和ffmpeg，无需手动安装
- **优化程序性能**：优化DPI缩放、信号处理、界面渲染等性能问题

### 2026年5月1日18:33:56 维护
- **使用安装程序解决扩展库不存在问题**：现在安装程序会自动释放bento4和ffmpeg，无需手动安装

### 2026年4月18日07:20:11 维护
- **收藏增加骨架屏，修复收藏获取不全问题**：现在几百的收藏也可以获取了
- **在缺少扩展时提示下载**:我将hevc扩展放在目录下hevc安装.appx，可以直接运行，程序内也支持安装
- **字幕解析研究中**：字幕功能暂时无法使用，后续版本继续研究
