...
- **新增CLI命令行工具**：CMD输入`bilibilidownloadtool`即可使用，GUI启动失败自动降级到CLI
- **新增程序启动自动申请管理员权限**（UAC提权）
- **新增安装程序欢迎页Markdown渲染**：更新日志正确显示格式
- **新增打包排除用户数据**：cookie/日志/历史不入安装包，覆盖安装自动保留
- **修复HEVC/AV1视频花屏**：.m4s文件不再跳过编码检测，未知编码自动转H.264
- **修复课程API缺少drm_tech_type参数**
- **修复收藏夹内容为空**：API返回null时正确处理，不再报NoneType错误
- **修复收藏夹课程/番剧不显示**：去掉type==2硬编码限制，支持所有收藏类型
- **修复Tab切换错位**：弹幕解析/收藏夹/封面/历史Tab索引修正
- **修复收藏夹卡片尺寸跳动**：统一卡片宽度+最大宽度约束
- **修复日志压缩后复制不到剪贴板**：剪切板操作移回主线程
- **修复安装程序路径写死**：覆盖安装检测改为检查目标目录而非注册表
- **优化剪贴板检测性能**：定时器1500ms+长度短路比较
- **热更新修复**：修复热更新功能校验失败问题
...
- **确认字幕无法获取**：由于auth_key获取方式未解决，字幕功能暂时无法使用，后续版本继续研究

### 2026年5月1日18:33:56 维护
- **使用安装程序解决解密库不存在问题**：现在安装程序会自动释放bento4和ffmpeg，无需手动安装
