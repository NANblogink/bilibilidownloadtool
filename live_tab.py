...
from PyQt5.QtWidgets import (
    ...
    QGridLayout, QSizePolicy, QSpacerItem, QSystemTrayIcon, QMenu
)
...
class LiveTab(QWidget):
    ...
    def __init__(self, config=None, parent=None):
        ...
        self.record_threads = {}  # task_id -> LiveRecordThread
        self._record_task_counter = 0
    ...
    def _create_info_tab(self):
        ...
        self.room_id_input.setMinimumHeight(scale(30))
        self.room_id_input.setStyleSheet(scale_style("padding: 4px 8px;"))
        ...
        self.cover_label.setFixedSize(scale(120), scale(80))
        ...
        self.quality_combo.setMinimumWidth(scale(100))
        ...
        self.play_in_player_btn.setStyleSheet(scale_style(
            "padding: 6px 14px; background-color: #e6a23c; color: white; border: none; border-radius: 4px; font-size: 12px;"))
        self.play_in_player_btn.setMinimumHeight(scale(30))
    ...
    def _get_live_stream(self):
        ...
        # 刷新Cookie，确保使用最新的登录状态获取高画质
        self._refresh_cookies()

        def worker():
            try:
                # 获取用户选择的画质，如果没有选择则使用最高画质
                qn = self.quality_combo.currentData()
                if qn is None:
                    qn = 10000  # 默认使用原画
                logger.info(f"开始获取直播流，房间号: {room_id}, 画质: {qn}")
                result = self.live_parser.get_live_stream_url(room_id, platform="h5", qn=qn)
    ...
    def _on_stream_done(self, success, data):
        ...
        try:
            ...
            # 更新画质选择
            # 保存用户之前的选择
            prev_qn = self.quality_combo.currentData()
            self.quality_combo.clear()
            quality_desc = data.get("quality_description", [])
            current_qn_found = False
            for q in quality_desc:
                qn_val = int(q.get("qn", 0))  # 转为整数
                desc = q.get("desc", "")
                self.quality_combo.addItem(f"{desc} ({qn_val})", qn_val)
                # 如果是用户之前选择的画质，标记为找到
                if prev_qn is not None and qn_val == prev_qn:
                    current_qn_found = True
                    self.quality_combo.setCurrentIndex(self.quality_combo.count() - 1)
            
            # 如果没有匹配用户之前的画质，选择当前实际获取到的画质
            if not current_qn_found:
                current_qn = data.get("current_qn")
                if current_qn:
                    for i in range(self.quality_combo.count()):
                        if self.quality_combo.itemData(i) == current_qn:
            ...
            if stream_urls:
                # V2接口返回更丰富的信息，优先展示HLS(fmp4)格式
                # 按格式排序：优先HLS > FLV，优先h264 > h265
                def _sort_key(u):
                    fmt = u.get("format", "")
                    codec = u.get("codec", "")
                    # HLS fmp4优先(2), HLS ts(1), FLV(0)
                    fmt_prio = 2 if "fmp4" in fmt else (1 if "ts" in fmt or "hls" in fmt.lower() else 0)
                    codec_prio = 0 if "avc" in codec or "h264" in codec else 1
                    return (-fmt_prio, -codec_prio)
                sorted_urls = sorted(stream_urls, key=_sort_key)
                url_text = "\n".join(
                    f"[{u.get('cdn_name', '?')}/{u.get('format', '?')}/{u.get('codec', '?')}] {u['url']}"
                    for u in sorted_urls
                )
                self.stream_url_text.setPlainText(url_text)
                self._current_stream_url = sorted_urls[0]["url"]
                # 保存实际返回的画质信息
                self._current_actual_qn = data.get("current_qn")
                self._current_requested_qn = prev_qn
            ...
            def refresher():
                try:
                    ...
                    if result.get("success"):
                        ...
                        if urls:
                            # 优先选HLS格式（更适合播放器）
                            hls_urls = [u for u in urls if "fmp4" in u.get("format", "") or "ts" in u.get("format", "")]
                            best = hls_urls[0] if hls_urls else urls[0]
                            new_url = best["url"]
    ...
    def _create_record_tab(self):
        ...
        settings_layout.setContentsMargins(scale(12), scale(16), scale(12), scale(12))
        settings_layout.setVerticalSpacing(scale(12))

        settings_layout.addWidget(QLabel("房间号:"), 0, 0)
        self.record_room_input = QLineEdit()
        self.record_room_input.setPlaceholderText("直播间房间号")
        self.record_room_input.setMinimumHeight(scale(32))
        self.record_room_input.setStyleSheet(scale_style("padding: 4px 8px;"))
        ...
        self.format_combo.setMinimumHeight(scale(32))
        self.format_combo.setStyleSheet(scale_style("padding: 2px 6px;"))
        ...
        # 按从高到低排列，与API qn参数一致
        self.record_quality_combo.addItem("杜比", 30000)
        self.record_quality_combo.addItem("4K", 20000)
        self.record_quality_combo.addItem("原画", 10000)
        ...
        self.record_quality_combo.setCurrentIndex(2)  # 默认选择原画
        self.record_quality_combo.setMinimumHeight(scale(32))
        self.record_quality_combo.setStyleSheet(scale_style("padding: 2px 6px;"))
        ...
        self.duration_spin.setMinimumHeight(scale(32))
        self.duration_spin.setToolTip("0 = 一直录制直到手动停止")
        duration_layout.addWidget(self.duration_spin)
        self.duration_label = QLabel("(0=手动停止)")
        self.duration_label.setStyleSheet(scale_style("font-size: 11px; color: #6c757d;"))
        ...
        self.save_path_input.setMinimumHeight(scale(32))
        ...
        self.browse_btn.setMinimumHeight(scale(32))
        self.browse_btn.setStyleSheet(scale_style("padding: 4px 12px; border: 1px solid #d9d9d9; border-radius: 4px; background: #fff;"))
        ...
        self.start_record_btn.setStyleSheet(scale_style(
            "padding: 8px 20px; background-color: #dc3545; color: white; border: none; border-radius: 4px; font-size: 13px; font-weight: bold;"))
        self.start_record_btn.setMinimumHeight(scale(36))
        ...
        self.pause_record_btn.setStyleSheet(scale_style(
            "padding: 8px 16px; background-color: #ffc107; color: #333; border: none; border-radius: 4px; font-size: 13px;"))
        self.pause_record_btn.setMinimumHeight(scale(36))
        ...
        self.stop_record_btn.setStyleSheet(scale_style(
            "padding: 8px 20px; background-color: #6c757d; color: white; border: none; border-radius: 4px; font-size: 13px;"))
        self.stop_record_btn.setMinimumHeight(scale(36))
        ...
        status_layout.setContentsMargins(scale(12), scale(16), scale(12), scale(12))

        self.record_status_label = QLabel("未开始录制")
        self.record_status_label.setStyleSheet(scale_style("font-size: 13px; font-weight: bold; color: #6c757d;"))
        ...
        self.record_time_label.setStyleSheet(scale_style("font-size: 24px; font-weight: bold; color: #00a1d6;"))
        ...
        self.record_progress.setMinimumHeight(scale(20))
    ...
    def _start_recording(self):
        ...
        def worker():
            ...
            # 优先选HLS格式用于录制
            hls_urls = [u for u in stream_urls if "fmp4" in u.get("format", "") or "ts" in u.get("format", "")]
            best_url_info = hls_urls[0] if hls_urls else stream_urls[0]
            stream_url = best_url_info["url"]
            ...
            def url_refresher():
                try:
                    ...
                    if result.get("success"):
                        urls = result.get("data", {}).get("stream_urls", [])
                        if urls:
                            hls = [u for u in urls if "fmp4" in u.get("format", "") or "ts" in u.get("format", "")]
                            best = hls[0] if hls else urls[0]
                            return best["url"]
                    return None
                except Exception as e:
                    logger.warning(f"[录制] 刷新URL失败: {e}")
    ...
    def _do_start_record(self, stream_url, output_path, format_type, duration):
        ...
        # 启动录制线程
        self._record_task_counter += 1
        task_id = self._record_task_counter
        thread = LiveRecordThread(
            stream_url, output_path, format_type, duration,
            url_refresher=url_refresher, room_id=room_id)
        thread.status_changed.connect(self._on_record_status)
        thread.progress_updated.connect(self._on_record_progress)
        thread.record_finished.connect(lambda ok, msg, tid=task_id: self._on_record_finished(ok, msg, tid))
        thread.error_occurred.connect(self._on_record_error_main)
        thread.start()
        self.record_threads[task_id] = thread
    ...
    def _on_record_finished(self, success, msg, task_id=None):
        if task_id:
            self.record_threads.pop(task_id, None)
        # 只有当没有其他录制任务时才更新按钮状态
        any_recording = any(t.isRunning() for t in self.record_threads.values())
    ...
    def _stop_recording(self):
        """停止录制（停止所有正在录制的任务）"""
        any_stopped = False
        for tid, t in list(self.record_threads.items()):
            if t.isRunning():
                t.stop()
                any_stopped = True
        if not any_stopped:
    ...
    def _pause_recording(self):
        """暂停/继续录制（对所有录制任务操作）"""
        running = [t for t in self.record_threads.values() if t.isRunning()]
        if not running:
            return
    ...
    def handle_tray_stop(self, session_id):
        """父窗口转发的：托盘请求停止录制"""
        for rid, t in self.record_threads.items():
            if t.isRunning():
                t.stop()
        self.stop_record_btn.setEnabled(False)
        self.pause_record_btn.setEnabled(False)
    ...
    def handle_tray_resume(self, session_id):
        """父窗口转发的：托盘请求继续录制"""
        for rid, t in self.record_threads.items():
            if t.isRunning() and t._pause_flag:
                t.resume()
        self.pause_record_btn.setText("暂停")
        self.pause_record_btn.setStyleSheet(scale_style(
            "padding: 10px 20px; background-color: #ffc107; color: #333; border: none; border-radius: 4px; font-size: 14px;"))
    ...
    def _create_replay_tab(self):
        ...
        self.refresh_replay_btn.setStyleSheet(scale_style(
            "padding: 6px 14px; background-color: #00a1d6; color: white; border: none; border-radius: 4px; font-size: 12px; font-weight: 500;"))
        self.refresh_replay_btn.setMinimumHeight(scale(32))
        ...
        self.load_other_replay_btn.setStyleSheet(scale_style(
            "padding: 6px 14px; background-color: #17a2b8; color: white; border: none; border-radius: 4px; font-size: 12px; font-weight: 500;"))
        self.load_other_replay_btn.setMinimumHeight(scale(32))
        ...
        self.replay_search_input = QLineEdit()
        self.replay_search_input.setPlaceholderText("过滤标题/直播时间/状态...")
        self.replay_search_input.setMinimumHeight(scale(28))
        self.replay_search_input.setStyleSheet(scale_style("padding: 4px 8px;"))
        self.replay_search_input.textChanged.connect(self._filter_replay_table)
        search_layout.addWidget(self.replay_search_input, stretch=1)
        self.replay_search_clear_btn = QPushButton("清除")
        self.replay_search_clear_btn.setStyleSheet(scale_style(
            "padding: 4px 12px; background-color: #6c757d; color: white; border: none; border-radius: 4px; font-size: 12px;"))
        self.replay_search_clear_btn.setMinimumHeight(scale(28))
        self.replay_search_clear_btn.clicked.connect(lambda: self.replay_search_input.clear())
        search_layout.addWidget(self.replay_search_clear_btn)
        ...
        dl_settings_layout.setSpacing(scale(10))
        dl_settings_layout.setContentsMargins(scale(10), scale(14), scale(10), scale(10))
        ...
        self.replay_save_path_input.setMinimumHeight(scale(32))
        self.replay_save_path_input.setStyleSheet(scale_style("padding: 4px 8px;"))
        ...
        self.replay_browse_btn.setStyleSheet(scale_style(
            "padding: 6px 14px; background-color: #6c757d; color: white; border: none; border-radius: 4px; font-size: 12px;"))
        self.replay_browse_btn.setMinimumHeight(scale(32))
        ...
        dl_layout.setSpacing(scale(10))
        dl_layout.setContentsMargins(scale(10), scale(14), scale(10), scale(10))

        self.replay_dl_status = QLabel("就绪")
        self.replay_dl_status.setStyleSheet(scale_style("font-size: 12px; color: #6c757d;"))
        dl_layout.addWidget(self.replay_dl_status)

        progress_layout = QHBoxLayout()
        progress_layout.setSpacing(scale(10))
        self.replay_dl_progress = QProgressBar()
        self.replay_dl_progress.setMinimumHeight(scale(20))
        progress_layout.addWidget(self.replay_dl_progress, stretch=1)
        ...
        self.replay_cancel_btn.setMinimumHeight(scale(32))
    ...
    def _on_replay_list_loaded(self, result):
        ...
        # 保存全量数据用于搜索过滤
        self._replay_all_list = list(replay_list or [])
        self._replay_data = {}
        # 重置搜索框（不触发过滤循环）
        if hasattr(self, 'replay_search_input'):
            self.replay_search_input.blockSignals(True)
            self.replay_search_input.clear()
            self.replay_search_input.blockSignals(False)
        self._populate_replay_table(replay_list or [])
    ...
    def _populate_replay_table(self, replay_list):
        """填充回放表格"""
    ...
    def _filter_replay_table(self):
        """按搜索关键词过滤回放表格"""
        all_list = getattr(self, '_replay_all_list', []) or []
        keyword = self.replay_search_input.text().strip().lower()
        if not keyword:
            self._populate_replay_table(all_list)
            self.status_label.setText(f"共 {len(all_list)} 条回放记录")
            return
        status_map = {0: "未生成", 2: "已完成", 30: "合成中"}
        filtered = []
        for replay in all_list:
            title = (replay.get("title", "") or "").lower()
            live_time = (replay.get("live_time", "") or "").lower()
            status_str = status_map.get(replay.get("replay_status", 0), "").lower()
            if keyword in title or keyword in live_time or keyword in status_str:
                filtered.append(replay)
        self._populate_replay_table(filtered)
        self.status_label.setText(f"过滤结果：{len(filtered)} / {len(all_list)} 条回放记录")

    def _show_replay_context_menu(self, pos):
        """回放表格右键菜单"""
        from PyQt5.QtWidgets import QApplication
        item = self.replay_table.itemAt(pos)
        if not item:
            return
        row = item.row()
        # 通过行号从全量数据取回放对象（过滤后行号与原始列表不一致）
        # 表格第4列是回放ID，用它在 _replay_data 中查
        id_item = self.replay_table.item(row, 4)
        if not id_item:
            return
        replay_id = id_item.text()
        replay = self._replay_data.get(replay_id) if hasattr(self, '_replay_data') else None
        if not replay:
            return
        menu = QMenu(self)
        title = replay.get("title", "")
        replay_id_val = replay.get("replay_id", "")
        status = replay.get("replay_status", 0)

        act_copy_title = menu.addAction("复制标题")
        act_copy_id = menu.addAction("复制回放ID")
        menu.addSeparator()
        act_play = menu.addAction("播放回放")
        # 仅已完成可下载
        if status == 2:
            act_dl_video = menu.addAction("下载画面")
            act_dl_audio = menu.addAction("下载音频")
            act_dl_full = menu.addAction("下载完整视频")
        else:
            act_dl_video = act_dl_audio = act_dl_full = None

        action = menu.exec_(self.replay_table.viewport().mapToGlobal(pos))
        if action == act_copy_title:
            QApplication.clipboard().setText(title)
            self.status_label.setText("已复制标题")
        elif action == act_copy_id:
            QApplication.clipboard().setText(str(replay_id_val))
            self.status_label.setText(f"已复制回放ID: {replay_id_val}")
        elif action == act_play:
            self._play_replay(replay)
        elif act_dl_video is not None and action == act_dl_video:
            self._download_replay(replay, 2)
        elif act_dl_audio is not None and action == act_dl_audio:
            self._download_replay(replay, 1)
        elif act_dl_full is not None and action == act_dl_full:
            self._download_replay(replay, 0)
