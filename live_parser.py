...
class LiveParser:
    ...
    def get_live_stream_url(self, room_id, platform="web", qn=None):
        """
        获取直播流地址（优先使用V2接口，支持Cookie画质选择）
        :param room_id: 真实房间号（长号）
        :param platform: web=http-flv, h5=hls(m3u8)
        :param qn: 画质代码，None=自动最高
        :return: 直播流URL列表
        """
        # 优先使用V2接口（能正确根据Cookie返回不同画质）
        v2_result = self._get_live_stream_url_v2(room_id, qn=qn)
        if v2_result["success"]:
            return v2_result

        # V2失败时回退到V1接口
        logger.warning(f"V2接口获取直播流失败，回退V1: {v2_result.get('error')}")
        return self._get_live_stream_url_v1(room_id, platform=platform, qn=qn)

    def _get_live_stream_url_v2(self, room_id, qn=None):
        """
        V2接口获取直播流（支持Cookie画质选择，返回HLS/FLV多格式）
        :param room_id: 真实房间号
        :param qn: 画质代码
        """
        params = {
            "room_id": room_id,
            "protocol": "0,1",       # 0=http-flv, 1=hls
            "format": "0,1,2",       # 0=flv, 1=ts(hls), 2=fmp4(hls)
            "codec": "0,1",          # 0=avc(h264), 1=hevc(h265)
            "platform": "web",
            "ptype": "16",
        }
        if qn:
        ...
        result = self._get("/xlive/web-room/v2/index/getRoomPlayInfo", params)
        if not result["success"]:
            return result

        playurl_info = result["data"].get("playurl_info", {})
        if not playurl_info:
            return {"success": False, "error": "V2接口未返回播放信息，可能未开播"}

        playurl = playurl_info.get("playurl", {})
        current_qn = playurl.get("current_qn")
        accept_qn = playurl.get("accept_qn", [])
        quality_desc = playurl.get("quality_description", [])
        ...
        for stream in stream_list:
            ...
            for fmt in stream.get("format", []):
                ...
                for codec in fmt.get("codec", []):
                    ...
                    codec_qn = codec.get("current_qn", current_qn)
                    ...
                    for url_info in codec.get("url_info", []):
                        ...
                        stream_urls.append({
                            ...
                            "codec": codec_name,
                            "qn": codec_qn,
                            "cdn_name": url_info.get("name", ""),
                        })

        return {
            "success": True,
            "data": {
                "current_quality": current_qn,
                "current_qn": current_qn,
                "accept_quality": [str(q) for q in accept_qn],
                "quality_description": quality_desc,
                "stream_urls": stream_urls,
            }
        }

    def _get_live_stream_url_v1(self, room_id, platform="web", qn=None):
        """
        V1接口获取直播流（备用，不支持Cookie画质选择）
        :param room_id: 真实房间号
        :param platform: web=http-flv, h5=hls(m3u8)
        :param qn: 画质代码
        """
    ...
    def get_best_live_stream(self, room_id, prefer_hls=True):
        ...
        :return: 直播流URL
        """
        result = self.get_live_stream_url(room_id)
        if not result["success"]:
            return result
        ...
        if prefer_hls:
            ...
            flv = [u for u in urls if "flv" in u.get("format", "").lower()]
            best = flv[0] if flv else urls[0]
        return {
            "success": True,
            "data": {
                "url": best["url"],
                "platform": best.get("protocol", ""),
    ...
    def get_area_list(self):
        ...
        :return: 分区列表（父分区 + 子分区）
        """
        result = self._get("/room/v1/Area/getList")
        if not result["success"]:
            return result
        data = result["data"]
        # 格式化分区数据
        ...
        for parent in data:
            ...
            sub_areas = parent.get("list", [])
            for sub in sub_areas:
                area_list.append({
                    "parent_id": parent_id,
                    "parent_name": parent_name,
                    "area_id": int(sub.get("id", 0)),
                    "area_name": sub.get("name", ""),
                    "hot_status": sub.get("hot_status", 0),
                })
        return {
            "success": True,
            "data": {
    ...
    def get_recommend_list(self, platform="web", web_location="333.1007", page=1):
        """
        获取主页直播推荐列表
        :param platform: 平台类型
        :param web_location: 网页位置代码
        :param page: 页码（从1开始）
        :return: 推荐直播间列表
        """
        result = self._get("/xlive/web-interface/v1/webMain/getMoreRecList", {
            "platform": platform,
        ...
        if not result["success"]:
            return result
        data = result["data"]
        recommend_list = data.get("recommend_room_list", [])
        return {
            "success": True,
            "data": {
                "recommend_list": [
                    {
                        "room_id": r.get("roomid", 0),
                        "title": r.get("title", ""),
                        "uname": r.get("uname", ""),
                        "face": r.get("face", ""),
                        "cover": r.get("cover", ""),
                        "online": r.get("online", 0),
                        "area_v2_id": r.get("area_v2_id", 0),
                        "area_v2_name": r.get("area_v2_name", ""),
                        "area_v2_parent_name": r.get("area_v2_parent_name", ""),
                        "link": r.get("link", ""),
                        "broadcast_type": r.get("broadcast_type", 0),
                    }
                    for r in recommend_list
                ],
                "top_room_id": data.get("top_room_id", 0),
            }
        }

    def get_hot_room_list(self, platform="web", parent_area_id=0, area_id=0, page=1, page_size=30):
        """
        获取热门直播间列表
        :param platform: 平台类型
        :param parent_area_id: 父分区ID（0表示全部分区）
        :param area_id: 子分区ID（0表示该父分区全部）
        :param page: 页码
        ...
        if parent_area_id > 0:
            params["parent_area_id"] = parent_area_id
        if area_id > 0:
            params["area_id"] = area_id
        result = self._get("/xlive/web-interface/v1/webMain/getHotRoom", params)
        return result
