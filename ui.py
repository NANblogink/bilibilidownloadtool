...
class BilibiliDownloader(BaseWindow):
    ...
    def init_ui(self):
        ...
        audio_quality_label.setMinimumWidth(60)
        audio_quality_label.setMinimumHeight(24)
        ...
        self.audio_quality_combo.setMinimumHeight(24)
    ...
    def create_window_and_start_downloads_wrapper(self, all_download_tasks, success_count, total_videos, space_info):
        ...
        try:
            ...
            for i, (_, _, _, download_params) in enumerate(all_download_tasks):
                logger.info(f"开始下载任务 {i+1}/{len(all_download_tasks)}：{download_params.get('task_id')}")
                self.download_manager.start_download(download_params)
                logger.info(f"任务 {i+1} 开始下载")
    ...
    def download_space_videos(self, videos, space_info, selected_quality=None, selected_audio_quality=None):
        ...
        class SpaceVideoParserThread(QThread):
            """空间视频解析线程"""
            finished = pyqtSignal(list, int, int, dict)  # 任务列表，成功数量，总数量，空间信息
            error = pyqtSignal(str)  # 错误信息
            progress = pyqtSignal(int, str)  # 进度信息
            ...
            def run(self):
                ...
                try:
                    ...
                    # 为每个视频创建下载任务
                    success_count = 0
                    max_videos = 50  # 限制一次处理的视频数量
                    total_videos = min(len(self.videos), max_videos)
                    logger.info(f"处理视频数量：{total_videos}")
                    
                    # 保存所有任务信息
                    all_download_tasks = []
                    
                    for i, video in enumerate(self.videos[:max_videos]):
                        try:
                            # 更新进度
                    ...
                    # 发送完成信号
                    self.finished.emit(all_download_tasks, success_count, len(self.videos), self.space_info)
        ...
        self.parser_thread.finished.connect(self.on_space_videos_parsed)
