...
class BilibiliParser:
            ...
            if response.status_code == 200:
                try:
                    ...
                    if script_match:
                        ...
                        # 提取视频列表
                        videos_match = re.search(r'list\s*:\s*\[(.*?)\]', script_content, re.DOTALL)
                        if videos_match:
                            videos_str = videos_match.group(1)
                            # 提取视频信息
                            video_pattern = re.compile(r'\{[^}]*?bvid[^}]*?\}', re.DOTALL)
    ...
    def _get_play_info(self, media_type, bvid, cid, is_tv_mode, season_id=None, ep_id=None):
        try:
            # 如果cid为空，尝试获取cid
            if not cid:
                try:
            ...
            if 'dash' in data_source:
                ...
                if 'audio' in data_source['dash'] and data_source['dash']['audio']:
                    ...
                    logger.debug(f"获取音频链接：{audio_url[:50]}...")
                ...
                if data_source['durl']:
                    ...
                    if not kid:
                        ...
                        if not kid:
                            # 从DURL链接中提取KID
                            kid_match = KID_REGEX['url_param'].search(video_url)
                            if kid_match:
                                kid = kid_match.group(1)
    ...
    def _extract_kid_from_m4s(self, m4s_path):
        ...
        try:
            ...
            bento4_path = os.path.join(self.current_dir, 'bento4', 'Bento4-SDK-1-6-0-641.x86_64-microsoft-win32', 'bin', 'mp4dump.exe')
            ...
            mp4info_path = os.path.join(self.current_dir, 'bento4', 'Bento4-SDK-1-6-0-641.x86_64-microsoft-win32', 'bin', 'mp4info.exe')
    ...
    async def _decrypt_with_bento4(self, input_file, output_file, kid=None):
        try:
            ...
            try:
                ...
                bento4_path = os.path.join(self.current_dir, 'bento4', 'Bento4-SDK-1-6-0-641.x86_64-microsoft-win32', 'bin', 'mp4decrypt.exe')
