...
class ApiRequest:
    def __init__(self, session=None):
        if session is None:
            ...
            # 扩大连接池以支持收藏夹分页等并发请求，避免 "Connection pool is full" 警告
            _adapter = HTTPAdapter(pool_connections=20, pool_maxsize=32, max_retries=0)
            ...
            self.session.trust_env = False  # 不读取系统代理，让开梯子也能直连B站
