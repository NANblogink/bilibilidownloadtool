...
from app_config import APP_NAME, APP_NAME_EN, APP_VERSION, SHORTCUT_NAME, is_safe_install_path
...
class UninstallPage(QWizardPage):
    ...
    def get_install_path(self):
        """卸载目标为 uninstaller.exe 同目录（即主程序所在目录），
        不读取环境变量或注册表路径，避免安装路径被改动后卸载错目录。"""
...
def is_installed():
    """检查软件是否已安装：以 uninstaller.exe 同目录为准，
    存在主程序可执行文件即视为已安装。"""
    exe_dir = os.path.dirname(os.path.abspath(sys.executable))
    main_exe = os.path.join(exe_dir, APP_NAME_EN + ".exe")
