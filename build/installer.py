...
import _pathsetup  # noqa: F401,E402  导入即完成子目录注入
...
class InstallerThread(QThread):
    ...
    def extract_package(self, package_path):
        try:
            ...
            self.ensure_env_tools()
            self.ensure_pyqt5_runtime()
    ...
    def ensure_env_tools(self):
        """在解压完成后，从 CDN 补装 ffmpeg/bento4 环境依赖（若包内未内嵌）。
        完整包已含工具则跳过；精简包缺工具时下载并安装到 _internal。"""
        internal_dir = os.path.join(self.install_path, '_internal')
        if not os.path.isdir(internal_dir):
            return

        has_ffmpeg = os.path.isfile(os.path.join(internal_dir, 'ffmpeg', 'bin', 'ffmpeg.exe'))
        has_bento4 = os.path.isfile(os.path.join(internal_dir, 'bento4', 'bin', 'mp4decrypt.exe'))
        if has_ffmpeg and has_bento4:
            self.log_signal.emit("环境依赖已随包内嵌，无需下载")
        else:
            self.log_signal.emit("正在下载并安装环境依赖(ffmpeg/bento4)，请稍候...")
            self.progress_signal.emit(92, 100)

            from tool_manager import install_tools_from_cdn

            def on_progress(pct, msg):
                if 92 <= int(pct) <= 99:
                    self.progress_signal.emit(int(92 + pct * 0.08), 100)
                self.log_signal.emit(msg)

            results = install_tools_from_cdn(
                internal_dir,
                need_ffmpeg=not has_ffmpeg,
                need_bento4=not has_bento4,
                progress_callback=on_progress,
            )
            ff_ok = has_ffmpeg or results.get('ffmpeg_installed', False)
            b4_ok = has_bento4 or results.get('bento4_installed', False)
            if not ff_ok or not b4_ok:
                missing = []
                if not ff_ok:
                    missing.append('ffmpeg')
                if not b4_ok:
                    missing.append('bento4')
                self.log_signal.emit(
                    f"提示: 环境依赖({'、'.join(missing)})下载失败，首次使用时应用将自动重试补装")

    def ensure_pyqt5_runtime(self):
        """精简包（内嵌无 PyQt5）安装阶段预下载 PyQt5，避免首次启动等待。
        完整包已内嵌 PyQt5 则跳过。失败不阻断安装，首次启动由 boot 再试。"""
        internal_dir = os.path.join(self.install_path, '_internal')
        if not os.path.isdir(internal_dir):
            return
        if os.path.isdir(os.path.join(internal_dir, 'PyQt5')):
            self.log_signal.emit("PyQt5 已随包内嵌，无需下载")
            return

        self.log_signal.emit("正在下载 PyQt5 运行环境（首次约70MB），请稍候...")
        self.progress_signal.emit(93, 100)
        try:
            import pyqt5_bootstrap
            target = os.path.join(internal_dir, 'pyqt5_runtime')
            ok = pyqt5_bootstrap.ensure_pyqt5(
                log=self.log_signal.emit, console=False, runtime=target)
            if ok:
                self.log_signal.emit("PyQt5 运行环境下载完成")
            else:
                self.log_signal.emit("提示: PyQt5 下载失败，首次启动时将自动重试")
        except Exception as e:
            self.log_signal.emit(f"提示: PyQt5 预下载失败({e})，首次启动时将自动重试")
