...
# 让 core/ 等子目录可导入，并从 app_config 读取权威版本号
if ROOT_DIR not in sys.path:
    sys.path.insert(0, ROOT_DIR)
import _pathsetup  # noqa: F401,E402

from app_config import VERSION_NUM  # noqa: E402
...
# 从 app_config 读取，避免此处版本号与项目实际版本脱节
APP_VERSION = f"{VERSION_NUM}.0"
...
STAGING_DIR = os.path.join(ROOT_DIR, "msix_staging")
ASSETS_DIR = os.path.join(STAGING_DIR, "Assets")
ONEFILE_DIR = os.path.join(ROOT_DIR, "dist_onefile")
...
def build_onefile_exe():
    ...
    def _stale(exe):
        ...
        watch = [
            os.path.join(ROOT_DIR, "core", "app_config.py"),
            os.path.join(SCRIPT_DIR, "build.py"),
            os.path.join(ROOT_DIR, "ui.py"),
            os.path.join(ROOT_DIR, "version_info.json"),
            os.path.join(ROOT_DIR, "logo.png"),
            os.path.join(ROOT_DIR, "assets", "images", "myqrcode.png"),
            os.path.join(ROOT_DIR, "assets", "images", "qunqrcode.png"),
            os.path.join(_ICONS_DIR := os.path.join(ROOT_DIR, "assets", "icons"), "qq_blue.svg"),
            os.path.join(ROOT_DIR, "main.py"),
            os.path.join(ROOT_DIR, "parsers", "bilibili_parser.py"),
        ]
        for w in watch:
    ...
    if "--force" in sys.argv or _stale(exe_path):
        for d in (ONEFILE_DIR, os.path.join(ROOT_DIR, "build_onefile")):
            if os.path.exists(d):
    ...
    cmd.extend([
        ...
        "--workpath", os.path.join(ROOT_DIR, "build_onefile"),
        "--specpath", os.path.join(ROOT_DIR, "build_dist"),
    ])
...
def copy_app_files():
    ...
    vi_src = os.path.join(ROOT_DIR, "version_info.json")
...
def sign_msix(msix_path):
    ...
    log(f"命令: {' '.join(cmd)}")
    result = subprocess.run(cmd, cwd=ROOT_DIR, capture_output=True, text=True, errors='replace')
    if result.returncode == 0:
        log("  本地签名成功")
    else:
