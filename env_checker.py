...
def _get_ffmpeg_executable_name():
    if IS_WINDOWS:
        return 'ffmpeg.exe'
    return 'ffmpeg'


def _get_ffprobe_executable_name():
    if IS_WINDOWS:
        return 'ffprobe.exe'
    return 'ffprobe'
...
def check_ffmpeg():
    ...
    ffmpeg_local = os.path.join(current_dir, 'ffmpeg', 'bin', _get_ffmpeg_executable_name())
    ...
    try:
        ...
        tool_ffmpeg = tool_manager.ffmpeg_path
        if os.path.exists(tool_ffmpeg):
            print(f"工具管理器中的ffmpeg: {tool_ffmpeg}")
            try:
                result = subprocess.run([tool_ffmpeg, '-version'], capture_output=True, text=False, timeout=10)
                if result.returncode == 0:
                    return True, tool_ffmpeg
            except Exception:
                pass
    except ImportError:
        pass
    ...
    print("未找到可用的FFmpeg")
    if IS_MACOS:
        print("提示: 可通过 Homebrew 安装 FFmpeg: brew install ffmpeg")
    return False, None
...
def fix_ffmpeg():
    ...
    ffmpeg_exec = os.path.join(ffmpeg_bin_path, _get_ffmpeg_executable_name())
