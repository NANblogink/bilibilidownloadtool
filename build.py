...
if os.path.exists(os.path.join(SCRIPT_DIR, 'warning_icon.png')):
    MAIN_EXTRA_ARGS.insert(-1, f"--add-data={os.path.join(SCRIPT_DIR, 'warning_icon.png')};.")
# 关于页面的 svg 图标（QQ/B站/GitHub/网站）位于 assets/icons
_ICONS_DIR = os.path.join(SCRIPT_DIR, "assets", "icons")
if os.path.isdir(_ICONS_DIR):
    MAIN_EXTRA_ARGS.insert(-1, f"--add-data={_ICONS_DIR};assets/icons")
# 关于页面的二维码图片（QQ交流群 / 作者QQ）
for _qr_img in ("myqrcode.png", "qunqrcode.png"):
    _qr_path = os.path.join(SCRIPT_DIR, _qr_img)
    if os.path.exists(_qr_path):
        MAIN_EXTRA_ARGS.insert(-1, f"--add-data={_qr_path};.")
...
LEAN_EXTRA_ARGS = [
    ...
    "--onedir",
    "--windowed",
    f"--icon={os.path.join(SCRIPT_DIR, 'logo.ico')}" if os.path.exists(os.path.join(SCRIPT_DIR, "logo.ico")) else "",
    "--hidden-import", "main",
    "--hidden-import", "cli",
    "--hidden-import", "live_parser",
    "--hidden-import", "live_tab",
...
if os.path.exists(os.path.join(SCRIPT_DIR, 'version_info.json')):
    LEAN_EXTRA_ARGS.append(f"--add-data={os.path.join(SCRIPT_DIR, 'version_info.json')};.")
if os.path.exists(os.path.join(SCRIPT_DIR, 'logo.ico')):
    LEAN_EXTRA_ARGS.append(f"--add-data={os.path.join(SCRIPT_DIR, 'logo.ico')};.")
if os.path.exists(os.path.join(SCRIPT_DIR, 'logo_alt_kaisui.ico')):
    LEAN_EXTRA_ARGS.append(f"--add-data={os.path.join(SCRIPT_DIR, 'logo_alt_kaisui.ico')};.")
if os.path.exists(os.path.join(SCRIPT_DIR, 'logo.png')):
    LEAN_EXTRA_ARGS.append(f"--add-data={os.path.join(SCRIPT_DIR, 'logo.png')};.")
if os.path.exists(os.path.join(SCRIPT_DIR, 'warning_icon.png')):
    LEAN_EXTRA_ARGS.append(f"--add-data={os.path.join(SCRIPT_DIR, 'warning_icon.png')};.")
_ICONS_DIR = os.path.join(SCRIPT_DIR, "assets", "icons")
if os.path.isdir(_ICONS_DIR):
...
UPX_EXE = os.path.join(SCRIPT_DIR, "upx_tool", "upx.exe")
...
def step2c_copy_lean_runtime():
    ...
    cer = os.path.join(SCRIPT_DIR, "BilibiliDownloader_dev.cer")
...
def generate_version_badge():
    badge_path = os.path.join(SCRIPT_DIR, "assets", "badges", "version.svg")
    label = "version"
    version = APP_VERSION
    char_w = {
