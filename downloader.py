                ...
                if download_content_type == 0:
                    ...
                    if batch_download_first and has_audio and download_content_type == 0:
                        ...
                        try:
                            ...
                            # audio_output_format 设置项生效：仅在有声视频模式下合并后提取音频轨
                            # 仅音频模式已直接输出音频文件，无需再次提取
                            if download_content_type == 0 and audio_path:
                                audio_format_setting = task_info.get('audio_format', 'mp3')
                                _audio_quality_id = task_info.get('audio_quality', 30280)
                                # Hi-Res无损(30251)/杜比全景声(30250)自动使用FLAC输出，避免无损转MP3变有损
                                if _audio_quality_id in (30251, 30250) and audio_format_setting.lower() in ('mp3', 'aac', 'ogg', 'm4a'):
                                    audio_format_setting = 'flac'
                                    logger.info(f"任务{task_id}：检测到Hi-Res/杜比音质，自动切换输出格式为FLAC以保持无损")
                                if audio_format_setting and audio_format_setting.lower() not in ('mp4', ''):
                                    try:
                                        audio_output_path = os.path.join(task_info['save_path'], f"{clean_title}.{audio_format_setting.lower()}")
                                        audio_output_path = os.path.abspath(audio_output_path)
                                        # 避免覆盖已有文件
                                        if not os.path.exists(audio_output_path):
                                            ffmpeg_exec = shutil.which('ffmpeg')
                                            if not ffmpeg_exec or not os.path.exists(ffmpeg_exec):
                                                ffmpeg_exec = task_info['parser'].ffmpeg_local if hasattr(task_info['parser'], 'ffmpeg_local') else 'ffmpeg'
                                            # FLAC：无损提取（Hi-Res源已是FLAC，直接copy）
                                            if audio_format_setting.lower() == 'flac':
                                                audio_cmd = [
                                                    ffmpeg_exec, '-i', output_path,
                                                    '-vn', '-acodec', 'copy',
                                                    '-y',
                                                    audio_output_path
                                                ]
                                            elif audio_format_setting.lower() == 'wav':
                                                audio_cmd = [
                                                    ffmpeg_exec, '-i', output_path,
                                                    '-vn', '-acodec', 'pcm_s16le',
                                                    '-y',
                                            ...
                                            elif audio_format_setting.lower() == 'mp3':
                                                audio_cmd = [
                                                    ffmpeg_exec, '-i', output_path,
                                            ...
                                            elif audio_format_setting.lower() in ('aac', 'm4a'):
                                                audio_cmd = [
                                                    ffmpeg_exec, '-i', output_path,
                                            ...
                                            elif audio_format_setting.lower() == 'ogg':
                                                audio_cmd = [
                                                    ffmpeg_exec, '-i', output_path,
                                                    '-vn', '-acodec', 'libvorbis', '-q:a', '5',
                                                    '-y',
                                                    audio_output_path
                                                ]
                                            else:
                                            ...
                                            audio_result = sp.run(audio_cmd, stdout=sp.PIPE, stderr=sp.PIPE, shell=False, **subprocess_no_window_kwargs(), timeout=120)
                                            if audio_result.returncode == 0 and os.path.exists(audio_output_path):
                                                logger.info(f"任务{task_id}：第{ep_index+1}集音频提取完成，格式：{audio_format_setting}")
                                            else:
                                                err = audio_result.stderr.decode('utf-8', errors='ignore')[:200] if audio_result.stderr else ''
                                                logger.warning(f"任务{task_id}：第{ep_index+1}集音频提取失败：{err}")
                                        ...
                                        logger.warning(f"任务{task_id}：第{ep_index+1}集音频提取异常：{audio_ex}")
