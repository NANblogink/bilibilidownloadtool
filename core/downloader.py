...
class EpisodeDownloadThread(QThread):
        ...
        try:
            ...
            if self.video_info.get('is_bangumi') and self.video_info.get('bangumi_info'):
                ...
                if actual_title:
                    ...
                    self.ep_title = f"第{self.ep_index+1}集"
            # [UP主] 文件名前缀（仅普通视频，先拼接再截断）
            if not self.video_info.get('is_bangumi') and not self.video_info.get('is_cheese'):
                if self.config and self.config.get_app_setting("filename_add_author", False):
                    author = (self.video_info.get('owner') or {}).get('name') or ''
                    if author:
                        self.ep_title = f"[{author}] {self.ep_title}"
            for c in illegal_filename_chars():
                self.ep_title = self.ep_title.replace(c, '_')
    ...
    def _check_save_path(self):
        try:
            ...
            self._apply_author_collection_subfolders()
    ...
    def _apply_author_collection_subfolders(self):
        """按设置追加 UP主文件夹 / 合集文件夹 子目录到保存路径下。
        仅普通视频生效；番剧/课程保持原有结构。异常时静默回退原路径。"""
        try:
            if self.video_info.get('is_bangumi') or self.video_info.get('is_cheese'):
                return
            use_author = self.config.get_app_setting("use_author_folder", False) if self.config else False
            use_collect = self.config.get_app_setting("use_collection_folder", False) if self.config else False
            if not use_author and not use_collect:
                return
            author = self._clean_segment((self.video_info.get('owner') or {}).get('name') or '')
            collect = self._clean_segment((self.video_info.get('ugc_season') or {}).get('title') or '')
            mode = self.config.get_app_setting("collection_folder_mode", "author") if self.config else "author"
            segments = []
            if use_collect and collect and mode == 'root':
                # 合集独立：只建合集夹
                segments.append(collect)
            else:
                if use_author and author:
                    segments.append(author)
                if use_collect and collect:
                    segments.append(collect)
            if not segments:
                return
            self.save_path = os.path.join(self.save_path, *segments)
        except Exception as e:
            logger.error(f"UP主/合集文件夹处理失败: {e}")

    def _clean_segment(self, s):
        s = (s or '').strip()
        if not s:
            return ''
        for c in illegal_filename_chars():
