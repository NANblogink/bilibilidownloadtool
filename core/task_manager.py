...
class TaskManager:
    ...
    def update_task_status(self, task_id, status, error_message="", task_data=None):
        ...
        if error_message and status == "failed":
            ...
            # 完成状态时，消息存入completion_message而非error_message
            updates["completion_message"] = error_message if error_message else ""
            updates["error_message"] = ""
        elif status in ("downloading", "pending", "cancelled"):
            updates["error_message"] = error_message if error_message else ""
        if task_data:
